...
def webhook():
    data = request.json
    try:
        # Hata veren kısmı burasıyla değiştiriyoruz
        tx = data[0]
        # İşlemi yapan cüzdanı bulmak için farklı yolları dene
        address = tx.get('feePayer') or tx.get('account') or tx['tokenTransfers'][0].get('fromUserAccount')
        ...
        market_cap = float(resp['pairs'][0].get('fdv', 0)) if 'pairs' in resp and len(resp['pairs']) > 0 else 0
        ...
        print(f"Hata detayı: {e}") # Hatanın ne olduğunu tam görelim
