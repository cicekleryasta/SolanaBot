...
wallet_history = {}
coin_buyers = {}  # coin_mint -> izlenen cüzdanlardan kaç tanesi aldı {address, address, ...}
...
def webhook():
    ...
    try:
        ...
        # Sadece izlediğimiz cüzdanlardan gelen işlemleri işle
        if address not in WALLET_NAMES:
            return "ok", 200
        ...
        if market_cap >= 10000:
            ...
            if direction == "ALIM 🟢":
                ...
                # coin_buyers takibi — her cüzdan sadece 1 kez sayılsın
                if memecoin_mint not in coin_buyers:
                    coin_buyers[memecoin_mint] = set()
                coin_buyers[memecoin_mint].add(address)

                # 3 farklı izlenen cüzdan aynı coini aldıysa özel bildirim
                buyer_count = len(coin_buyers[memecoin_mint])
                if buyer_count == 3:
                    buyer_names = ", ".join(
                        WALLET_NAMES.get(a, a[:8] + "...") for a in coin_buyers[memecoin_mint]
                    )
                    alert_msg = (f"🔥 KONVOY ALARMI 🔥\n"
                                 f"3 izlenen cüzdan aynı coini aldı!\n"
                                 f"🪙 Coin: {coin_name} (${coin_symbol})\n"
                                 f"💰 Market Cap: ${market_cap:,.0f}\n"
                                 f"👥 Alanlar: {buyer_names}\n"
                                 f"📊 Grafik: https://dexscreener.com/solana/{memecoin_mint}")
                    alert_url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage?chat_id={CHAT_ID}&text={alert_msg}&disable_web_page_preview=true"
